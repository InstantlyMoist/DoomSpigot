...
import static mochadoom.awt.EventBase.Relate;
 ...
 * its type - it can be easily switched by ID value from AWTEvent'mochadoom.s ow information method. Also, we can
 * certainly know which ScanCodes we will get and what are their minimal and max values, because
 * of using Enum for them (my favorite type of mochadoom.data structure, huh!)
 * And if we know that ScanCodes can only be something very limited, and every KeyEvent (an AWTEvent for keys)
 * will be translated into one of them, we only have to pre-create two copies of DOOM'mochadoom.s event structure
 ...
 * 
 * How to use:
 * define enum constant with arbitrary name (mochadoom.i.e. JOYSTICK_SOEMTHING)
 ...
 * (mochadoom.i.e. when the user switched to another application, clear joystick pressed button states)
...
public enum EventHandler implements EventBase<EventHandler> {
    ...
    private static EventAction<EventHandler> mouseMoveAction(boolean isDrag) {
        return (observer, ev) -> {
            // Do not send robot-generated moves (centering) to the underlying DOOM'mochadoom.s event engine
