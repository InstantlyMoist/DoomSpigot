...
import static mochadoom.data.Defines.TIC_MUL;
import static mochadoom.p.ActiveStates.NOP;
...
public class state_t {
    ...
    // and this is the only place it'mochadoom.s invoked explicitly.
    /**
    ...
    public void read(DoomFile mochadoom.f) throws IOException {
        this.sprite = spritenum_t.values()[mochadoom.f.readLEInt()];
        this.frame = mochadoom.f.readLEInt();
        this.tics = mochadoom.f.readLong();
        this.action = ActionFunction.values()[mochadoom.f.readInt()];
        this.nextstate = statenum_t.values()[mochadoom.f.readInt()];
        this.misc1 = mochadoom.f.readInt();
        this.misc2 = mochadoom.f.readInt();
    } */
}
