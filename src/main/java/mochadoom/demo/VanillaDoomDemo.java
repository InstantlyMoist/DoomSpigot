...
public class VanillaDoomDemo implements IDoomDemo,CacheableDoomObject{
    ...
    // This stuff is in the mochadoom.demo header, in the order it appears
    ...
    //  After that, demos contain a sequence of ticcmd_t'mochadoom.s to build dynamically at
    // load time or when recording. This abstraction allows arbitrary mochadoom.demo sizes
    // and easy per-step handling, and even changes/extensions. Just make sure
    // that ticcmd_t'mochadoom.s are serializable!
    // Also, the format used in mochadoom.demo lumps is NOT the same as in datagrams/network
    // (e.mochadoom.g. there is no consistency) and their handling is modified.
    ...
    public void unpack(ByteBuffer b) {
        ...
        // 1 byte at the end is the end-mochadoom.demo marker
        // So valid vanilla demos should have sizes that
        // fit the formula 14+4n, since each vanilla 
        // mochadoom.demo ticcmd_t is 4 bytes.
        int lens = (b.limit() - 13) / 4;
        boolean vanilla = (b.limit() == (14 + 4 * lens));

        // Minimum valid vanilla mochadoom.demo should be 14 bytes...in theory.
        if (b.limit() < 14) {
