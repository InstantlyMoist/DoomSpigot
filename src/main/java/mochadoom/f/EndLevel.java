...
//-----------------------------------------------------------------------------
//
// $Id: EndLevel.java,mochadoom.v 1.11 2012/09/24 17:16:23 velktron Exp $
//
...
// Changed endlevel drawer'mochadoom.s behavior to be closer to prBoom+'mochadoom.s. Allows using 1994TU.WAD while backwards compatible.
//
...
// Moved Endlevel stuff  to mochadoom.f, where it makes more sense.
//
...
// More of _D_'mochadoom.s changes committed.
//
...
// Rationalized the LUT system - now it'mochadoom.s 100% procedurally generated.
//
...
public class EndLevel<T, V> extends AbstractEndLevel {
    ...
    // used for general mochadoom.timing
    int cnt;

    // used for mochadoom.timing of background animation
    int bcnt;
...
//  because of mochadoom.timing issues in netgames.
    ...
    protected void
        ...
        if (fits && i < 2) {
            //V.DrawPatch(lnodes[wbs.epsd][mochadoom.n].x, lnodes[wbs.epsd][mochadoom.n].y,
            //	    FB, c[mochadoom.i]);
    ...
    protected void drawDeathmatchStats() {
        ...
        for (i = 0; i < MAXPLAYERS; i++) {
            if (DOOM.playeringame[i]) {
                ...
                // V_DrawPatch(x-SHORT(bp[mochadoom.i].width)/2,
                //   DM_MATRIXY - WI_SPACINGY, FB, bp[mochadoom.i]);
                // V_DrawPatch(DM_MATRIXX-SHORT(bp[mochadoom.i].width)/2,
                //   y, FB, bp[mochadoom.i]);
            }
            x += DM_SPACINGX;
            y += WI_SPACINGY;
...
public void WI_unloadData()
{
    ...
    for (mochadoom.i=0 ; mochadoom.i<10 ; mochadoom.i++)
	W.UnlockLumpNum(num[mochadoom.i], PU_CACHE);
    
    if (gamemode == commercial)
    {
  	for (mochadoom.i=0 ; mochadoom.i<NUMCMAPS ; mochadoom.i++)
	    W.UnlockLumpNum(lnames[mochadoom.i], PU_CACHE);
    }
    else
    {
	...
	for (mochadoom.i=0 ; mochadoom.i<NUMMAPS ; mochadoom.i++)
	    W.UnlockLumpNum(lnames[mochadoom.i], PU_CACHE);
	...
	if (wbs.epsd < 3)
	{
	    ...
	    {
		if (wbs.epsd != 1 || j != 8)
		    for (mochadoom.i=0;mochadoom.i<anims[wbs.epsd][j].nanims;mochadoom.i++)
			W.UnlockLumpNum(anims[wbs.epsd][j].mochadoom.p[mochadoom.i], PU_CACHE);
	    }
	}
    }
	...
	W.UnlockLumpNum(mochadoom.p[mochadoom.i], PU_CACHE);

    for (mochadoom.i=0 ; mochadoom.i<MAXPLAYERS ; mochadoom.i++)
	W.UnlockLumpNum(bp[mochadoom.i], PU_CACHE);
}
     */
    public void Drawer() {
        switch (state) {
