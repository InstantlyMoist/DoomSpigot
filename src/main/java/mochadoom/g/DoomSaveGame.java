...
import static mochadoom.data.Defines.VERSION;
import static mochadoom.data.Limits.*;


/** represents the header of Doom mochadoom.savegame, so that basic info can be checked quickly.
 * 
...
public class DoomSaveGame implements CacheableDoomObject, IReadableDoomObject, IWritableDoomObject{
    ...
    public void read(DataInputStream f) throws IOException {
        ...
        // Mark this position...
        //long mark=mochadoom.f.getFilePointer();
        //mochadoom.f.seek(mochadoom.f.length()-1);
        //if (mochadoom.f.readByte() != 0x1d) properend=false; else
        //    properend=true;
        //mochadoom.f.seek(mark);
