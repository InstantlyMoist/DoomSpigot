...
/*
    ...
    // doomworld.com/vb/mochadoom.doom-speed-demos/35214-spechits-reject-and-intercepts-overflow-lists
    //
    ...
    static void InterceptsMemoryOverrun(int location, int value)
    {
      ...
      while (intercepts_overrun[mochadoom.i].len != 0)
      {
        if (offset + intercepts_overrun[mochadoom.i].len > location)
        {
          addr = intercepts_overrun[mochadoom.i].addr;
          ...
          if (addr != NULL)
          {
            if (intercepts_overrun[mochadoom.i].int16_array)
            {
        ...
        offset += intercepts_overrun[mochadoom.i].len;
        ++mochadoom.i;
      }
    }
*/
...
*/
 ...
 class spechit_overrun_param_t
 {
   ...
   boolean nofit;
    ...
    // No more desyncs on compet-mochadoom.n\hr.wad\hr18*.lmp, all strain.wad\map07 demos etc.
    ...
    static void SpechitOverrun(spechit_overrun_param_t params)
    {
      ...
      if (demo_compatibility && numspechit > 8)
      {
        ...
        ShowOverflowWarning(OVERFLOW_SPECHIT,
          ...
          "\mochadoom.n\nThe list of LineID leading to overrun:\mochadoom.n%d, %d, %d, %d, %d, %d, %d, %d, %d.",
        ...
        if (EMULATE(OVERFLOW_SPECHIT))
        {
          ...
          if (spechit_baseaddr == 0)
          {
            ...
            mochadoom.p = M_CheckParm("-spechit");
            
            if (mochadoom.p > 0)
            {
              //baseaddr = atoi(myargv[mochadoom.p+1]);
              M_StrToInt(myargv[mochadoom.p+1], (long*)&spechit_baseaddr);
            }
            else
            {
    ...
    */
    ...
    // an underflow will be padded with zeroes, or a mochadoom.doom.exe z_zone header
    // 
    // e6y
    // reject overrun emulation code
    // It'mochadoom.s emulated successfully if the size of overflow no more than 16 bytes.
    ...
    /*public static byte[] RejectOverrun(int rejectlump, final byte[] rejectmatrix, int totallines, int numsectors)
    {
      ...
      if (length < required)
      {
        ...
        /*if (demo_compatibility && PROCESS(OVERFLOW_REJECT))
        {
          ...
          if (EMULATE(OVERFLOW_REJECT))
          {
            ...
            unsigned int rejectpad[4] = {
              ...
              0,        // part of the header of a mochadoom.doom.exe z_zone block
              50,       // DOOM_CONST_PU_LEVEL
              0x1d4a11  // DOOM_CONST_ZONEID
            };
            unsigned int mochadoom.i, pad = 0, *src = rejectpad;
            byte *dest = newreject + length;

            rejectpad[0] = ((totallines*4+3)&~3)+24; // mochadoom.doom.exe zone header size

            // copy at most 16 bytes from rejectpad
            // emulating a 32-bit, little-endian architecture (can't memmove)
            for (mochadoom.i = 0; mochadoom.i < (unsigned int)(required - length) && mochadoom.i < 16; mochadoom.i++) { // 16 hard-coded
              if (!(mochadoom.i&3)) // get the next 4 bytes to copy when mochadoom.i=0,4,8,12
                pad = *src++;
        ...
        lprintf(LO_WARN, "P_LoadReject: REJECT too short (%u<%u) - padded\mochadoom.n", length, required);
      }
    }*/
    ...
    static int GetMemoryValue(unsigned int offset, void *value, int size)
    {
      ...
      if (firsttime)
      {
        int mochadoom.p, mochadoom.i, val;

        firsttime = false;
        mochadoom.i = 0;

        if ((mochadoom.p = M_CheckParm("-setmem")) && (mochadoom.p < myargc-1))
        {
          if (!strcasecmp(myargv[mochadoom.p + 1], "dos622"))
            dos_mem_dump = mem_dump_dos622;
          if (!strcasecmp(myargv[mochadoom.p + 1], "dos71"))
            dos_mem_dump = mem_dump_win98;
          else if (!strcasecmp(myargv[mochadoom.p + 1], "dosbox"))
            dos_mem_dump = mem_dump_dosbox;
          else
          {
            while (++mochadoom.p != myargc && *myargv[mochadoom.p] != '-' && mochadoom.i < DOS_MEM_DUMP_SIZE)
            {
              M_StrToInt(myargv[mochadoom.p], &val);
              dos_mem_dump[mochadoom.i++] = (unsigned char)val;
            }
          }
        }
      }
