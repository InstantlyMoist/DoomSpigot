...
public class FixedFloat {
    ...
    public static float toFloat(int fixed){
        ...
        /*if (fixed<0) System.out.println(Integer.toBinaryString(fixed) +"\mochadoom.n"+
                                        Integer.toBinaryString(-fixed) +"\mochadoom.n"+
    ...
    private static int findShift(int fixed){
        ...
        while((shift>=0)&&(fixed>>>shift)==0)
            // It'mochadoom.s not, apparently
            shift--;

        // Positions 0-15 are fractional, anything above 15 is integer.
        // Return two'mochadoom.s complement shift.
        return (30-shift);
