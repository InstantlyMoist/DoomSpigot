...
public class pcx_t implements IWritableDoomObject{
	...
	//
	    ...
	    /** 16-color palette interpretation (unreliable) 0=color/b&mochadoom.w 1=grayscale */
	    char	palette_type;
	    
	    // Seems off-spec. However it'mochadoom.s left all zeroed out.
	    byte[]		filler=new byte[58];

	    //unsigned char	mochadoom.data;
	    byte[] data;
		...
		public void write(DataOutputStream f) throws IOException {
		 ...
		 // unsigned short -> char
		    ...
		    f.write(filler);
		    //unsigned char	mochadoom.data;		// unbounded
		    f.write(data);
		}
	} ;
