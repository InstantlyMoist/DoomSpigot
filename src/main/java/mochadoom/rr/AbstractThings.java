...
import mochadoom.rr.drawfuns.ColumnFunction;
import mochadoom.v.graphics.Palettes;
import mochadoom.v.scale.VideoScale;
import mochadoom.v.tables.LightsAndColors;
import mochadoom.w.IWadLoader;

import static mochadoom.data.Defines.*;
import static mochadoom.doom.player_t.NUMPSPRITES;
import static mochadoom.m.fixed_t.*;
import static mochadoom.p.mobj_t.MF_TRANSLATION;
import static mochadoom.rr.line_t.ML_DONTPEGBOTTOM;

/**
 * Refresh of things, mochadoom.i.e. objects represented by sprites. This abstract
 ...
 * it'mochadoom.s a PITA to keep "disembodied". Then again, this probably means it'mochadoom.s more
 * extensible...
 * 
 * 
 * 
...
public abstract class AbstractThings<T,V> implements IMaskedDrawer<T,V> {
    ...
    protected void RenderMaskedSegRange(drawseg_t ds, int x1, int x2) {
        ...
        // System.out.printf("RenderMaskedSegRange from %d to %d\mochadoom.n",x1,x2);
        ...
        // System.out.print(" for texture "+textures[texnum].name+"\mochadoom.n:");
        lightnum =
    ...
    protected final void DrawSprite(vissprite_t<V> spr) {
        ...
        for (x = spr.x1; x <= spr.x2; x++) {
            ...
            // ?? What'mochadoom.s this bullshit?
            if (cliptop[x] == -2)
                cliptop[x] = -1;
        }
    ...
    public void DrawMasked() {
        ...
        // The original sort. It'mochadoom.s incredibly bad on so many levels (uses a
        // separate
        // linked list for the sorted sequence, which is pointless since the
        // vissprite_t
        // array is gonna be changed all over in the next frame anyway, it'mochadoom.s
        // not like
     ...
     * means: partly transparent, mochadoom.i.e. stored in posts/runs of opaque
   ...
   /* protected final void DrawMaskedColumn(T column) {
        ...
        // That'mochadoom.s true for the whole column.
        ...
        while ((topdelta = 0xFF & column[pointer]) != 0xFF) {
            ...
            if (maskedcvars.dc_yl <= maskedcvars.dc_yh
                ...
                // Set pointer inside column to current post'mochadoom.s mochadoom.data
                // Rremember, it goes {postlen}{postdelta}{pad}[mochadoom.data]{pad}
    ...
    */
    ...
    protected final void DrawMaskedColumn(column_t column) {
        ...
        for (int i = 0; i < column.posts; i++) {
            ...
            if (maskedcvars.dc_yl <= maskedcvars.dc_yh
                ...
                // Set pointer inside column to current post'mochadoom.s mochadoom.data
                // Remember, it goes {postlen}{postdelta}{pad}[mochadoom.data]{pad}
     ...
     * Masked means: partly transparent, mochadoom.i.e. stored
