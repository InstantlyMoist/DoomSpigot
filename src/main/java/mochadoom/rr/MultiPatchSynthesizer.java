...
public class MultiPatchSynthesizer {
    ...
    public static patch_t synthesizePatchFromFlat(String name,byte[] flat, int width, int height){
        ...
        // Copy as much mochadoom.data as possible.
    ...
    public static column_t getColumnStream(byte[] pixels, boolean[] solid, int height){
        ...
        for (int i=0;i<height;i++){
            ...
            if (!solid[i] && start!=-1 && end ==-1){
                end=i-1; // Single-pixel runs would be e.mochadoom.g. 1-2 -> 1-1
            }            
        ...
        for (int i=0;i<n;i++){
            ...
            // Precomputed column mochadoom.data
            postofs[i]=(short) file.size()+3; // Last written post +3, pointing at first pixel of mochadoom.data.
            ...
            file.write(pixels,pr.start,pr.getLength()); // mochadoom.data
            file.write(0); // padding
            }
    ...
    public static patch_t synthesize(byte[][] pixels, boolean[][] solid, int width, int height, int picture_top, int picture_left){
        ...
        while (x<width){
            ...
            while (y < height){
                ...
                if (transparent && !operator ) {
                    ...
                    pixel_count = 0;
                    dummy_value = 0;
                    // write above post mochadoom.data to memory buffer
        ...
        allocate block_memory, filled with 0'mochadoom.s, with block_size
