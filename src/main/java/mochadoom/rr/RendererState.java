...
import mochadoom.m.IDoomMenu;
import mochadoom.m.MenuMisc;
import mochadoom.m.Settings;
import mochadoom.Engine;
import mochadoom.p.mobj_t;
import mochadoom.rr.drawfuns.*;
import mochadoom.utils.C2JUtils;
import mochadoom.v.graphics.Palettes;
import mochadoom.v.renderers.DoomScreen;
import mochadoom.v.tables.BlurryTable;
...
import static mochadoom.data.Limits.*;
import static mochadoom.data.Tables.*;
import static mochadoom.doom.SourceCode.R_Draw.R_FillBackScreen;
import static mochadoom.m.BBox.*;
import static mochadoom.m.fixed_t.*;
import static mochadoom.p.ActiveStates.P_MobjThinker;
import static mochadoom.rr.line_t.*;
import static mochadoom.utils.GenericCopy.malloc;
import static mochadoom.v.DoomGraphicSystem.*;
import static mochadoom.v.renderers.DoomScreen.*;
 ...
 * implementations of the Doom'mochadoom.s renderer. Not the cleanest or more OO way
...
public abstract class RendererState<T, V> implements SceneRenderer<T, V>, ILimitResettable {
    ...
    public RendererState(DoomMain<T, V> DOOM) {
        ...
        // It'mochadoom.s better to construct this here
    ...
    protected final class BSP extends BSPVars {
        ...
        private void ClipSolidWallSegment(int first, int last) {
            ...
            { // crunch code
                ...
                while (next++ != newend) {
                    // Remove a post.
                    // MAES: this is a struct copy.
                    // MAES: this can overflow, breaking e.mochadoom.g. MAP30 of Final
                    // Doom.
                    if (next >= solidsegs.length) {
                        ResizeSolidSegs();
                    }
        ...
        // Does handle windows,
        // e.mochadoom.g. LineDefs with upper and lower texture.
        //
         ...
         * first two solidsegs, which are "neutered". It'mochadoom.s interesting to note
         ...
         * (line) visplane clipping should be performed. E.mochadoom.g. window, final
        ...
        private void Subsector(int num) {
            ...
            if (frontsector.floorheight < view.z) {
                ...
                // It'mochadoom.s never checked explicitly for either condition, just
                // called straight.
    ...
    protected abstract class SegDrawer implements ISegDrawer {
        ...
        public void StoreWallRange(int start, int stop) {
            ...
            /*
             * MAES: ok, this is a tricky spot. angle_t'mochadoom.s are supposed to be
            ...
            /*
             * This is the only place it'mochadoom.s ever explicitly assigned. Therefore
         ...
         * version. It'mochadoom.s the only difference between the parallel and serial
    ...
    protected interface IPlaneDrawer {
        ...
        /**
         * Sync up in case there'mochadoom.s concurrent planes/walls rendering
         */
        void sync();
    }
    ...
    protected interface ISegDrawer extends ILimitResettable {
         ...
         * e.mochadoom.g. parallel rendering
         */
        ...
        public void DrawPlanes() {
            ...
            for (int pl = 0; pl < vp_vars.lastvisplane; pl++) {
                ...
                if (pln.picnum == TexMan.getSkyFlatNum()) {
                    ...
                    /**
                     * Sky is allways drawn full bright, mochadoom.i.e. colormaps[0] is
                ...
                // We set those values at the border of a plane'mochadoom.s top to a
                // "sentinel" value...ok.
    ...
    /*
     * MAES: what'mochadoom.s going on with light tables here. OK...so these should be
     ...
     * pure function, as it alters the renderer'mochadoom.s state!
     */
    @Override
     ...
     * tables.c if (false){ int mochadoom.i; long t; float mochadoom.f; // // slope (tangent) to
     * angle lookup // for (mochadoom.i=0 ; mochadoom.i<=SLOPERANGE ; mochadoom.i++) { mochadoom.f = (float) Math.atan(
     * (double)(mochadoom.i/SLOPERANGE )/(3.141592657*2)); t = (long) (0xffffffffL*mochadoom.f);
     * tantoangle[mochadoom.i] = (int) t; } } }
     */
    /**
    ...
    protected void InitTables() {
        ...
        /*
         * int mochadoom.i; float a; float fv; int t; // viewangle tangent table for (mochadoom.i=0
         * ; mochadoom.i<FINEANGLES/2 ; mochadoom.i++) { a = (mochadoom.i-FINEANGLES/4+0.5)*PI*2/FINEANGLES;
         * fv = FRACUNIT*tan (a); t = fv; finetangent[mochadoom.i] = t; } // finesine
         * table for (mochadoom.i=0 ; mochadoom.i<5*FINEANGLES/4 ; mochadoom.i++) { // OPTIMIZE: mirro.. a =
         * (mochadoom.i+0.5)*PI*2/FINEANGLES; t = FRACUNIT*sin (a); finesine[mochadoom.i] = t; }
         */
    ...
    protected int PointToDist(int x, int y) {
         ...
         * change much. It'mochadoom.s better to be safe than sorry.
         */
    ...
    public void ExecuteSetViewSize() {
        ...
        for (int i = 0; i < view.height; i++) {
            ...
            // MyPlanes.yslopef[mochadoom.i] = ((viewwidth<<detailshift)/2)/ dy;
        }

        // double cosadjf;
        for (int i = 0; i < view.width; i++) {
            // MAES: In this spot we must interpet it as SIGNED, else it'mochadoom.s
            // pointless, right?
            // MAES: this spot caused the "warped floor bug", now fixed. Don't
            // forget xtoviewangle[mochadoom.i]!
            cosadj = Math.abs(finecosine(view.xtoviewangle[i]));
            // cosadjf =
            // Math.abs(Math.cos((double)xtoviewangle[mochadoom.i]/(double)0xFFFFFFFFL));
            MyPlanes.getDistScale()[i] = FixedDiv(FRACUNIT, cosadj);
            // MyPlanes.distscalef[mochadoom.i] = (float) (1.0/cosadjf);
        }
    ...
    protected void InitBuffer(int width, int height) {
        ...
        // Handle resize,
        // e.mochadoom.g. smaller view windows
    ...
    /**
     * R_InitTextureMapping Not moved into the TextureManager because it'mochadoom.s
    ...
    protected void InitLightTables() {
        ...
        for (i = 0; i < colormaps.lightLevels(); i++) {
            ...
            for (j = 0; j < colormaps.maxLightZ(); j++) {
                ...
                // zlight[mochadoom.i][j] = colormaps + level*256;
    ...
    protected void R_InitTranMap(int progress) {
        ...
        { // Compose a default transparent filter map based on PLAYPAL.
            System.out.print("Computing translucency map from scratch...that'mochadoom.s gonna be SLOW...");
            ...
            // The interpretation is that e.mochadoom.g. the mixture of color 2 and 8 will
            // have a RGB value, which is closest to euclidean distance to
            // e.mochadoom.g. original color 9. Therefore we should put "9" in the (2,8)
     ...
     * That'mochadoom.s true for the whole column. dc_source = column; // for each post...
     ...
     * pointer inside column to current post'mochadoom.s mochadoom.data // Rremember, it goes
     * {postlen}{postdelta}{pad}[mochadoom.data]{pad} dc_source_ofs = 0; // pointer + 3;
     ...
     * System.err.printf("Error rendering %d %d %d\mochadoom.n", dc_yl,dc_yh,dc_yh-dc_yl);
     ...
     * It'mochadoom.s an ugly hack, in that it must communicate with the "Game map" class
     ...
     * it'mochadoom.s going to be ugly and not neatly separated anyway.
     *
     * @return
     */
    @Override
     ...
     * same dcvars/dsvars object. Any initializations of e.mochadoom.g. parallel renderers
    ...
    public void RenderPlayerView(player_t player) {
        ...
        // Viewing variables are set according to the player'mochadoom.s mobj. Interesting
        // hacks like
