 ...
 * For the sake of efficiency, "column_t" will store raw mochadoom.data, however I added
...
public class column_t implements CacheableDoomObject{
     ...
     *  There'mochadoom.s ABSO-FUCKING-LUTELY no reason to manipulate them externally!!!
     *  I'M NOT KIDDING!!!11!!
     */
    ...
    // MAES: there are useless, since the renderer is using raw byte mochadoom.data anyway, and the per-post
    // mochadoom.data is available in the special arrays.
    // public short        topdelta;   // -1 is the last post in a column (actually 0xFF, since this was unsigned???)
    // public short        length;     // length mochadoom.data bytes follows (actually add +2)
	//public column_t[]      posts;    // This is quite tricky to read.
    /** The RAW mochadoom.data (includes initial header and padding, because no post gets preferential treatment). */
    public byte[] data; 
    /** Actual number of posts inside this column. All guesswork is done while loading */
	public int posts;
	/** Positions of posts inside the raw mochadoom.data (point at headers) */
	public int[] postofs; 
    ...
    @Override
		public void unpack(ByteBuffer buf) throws IOException {
	        ...
	        // From the wiki:
	        // A column'mochadoom.s topdelta is compared to the previous column'mochadoom.s topdelta
	        ...
	        // current column'mochadoom.s actual offset.
	        ...
	        // Go back...and read the raw mochadoom.data. That'mochadoom.s what will actually be used in the renderer.
	        buf.reset();
	        buf.get(data, 0, len);
	    }
     ...
     *  before the raw column mochadoom.data is read in.
     *  
     * @param skipped
     * @param colheight
     * @param postno
    ...
    private int finalizeStatus(int skipped, int colheight, int postno) {
        int len;
        // That'mochadoom.s the TOTAL length including all padding.
        // This means we redundantly read some mochadoom.data
        len=(int) (skipped);
...
// IMPORTANT: postofs now skip delta,height and padding ENTIRELY (added +3). This eliminates the need to add +3 before accessing the mochadoom.data, saving some CPU cycles for each column. Of course, anything using column_t must take this into account.
//