package mochadoom.rr.parallel;

import mochadoom.data.Tables;
import mochadoom.doom.DoomMain;
import mochadoom.rr.PlaneDrawer;
import mochadoom.rr.RendererState;
...
import static mochadoom.data.Tables.finetangent;
...
public abstract class AbstractParallelRenderer<T, V> extends RendererState<T, V> implements RWI.Init<T, V> {
    ...
    protected final class ParallelSegs extends SegDrawer implements RWI.Get<T, V> {
        ...
        /**
         * Parallel version. Since there'mochadoom.s so much crap to take into account
        ...
        public void CompleteRendering() {
            ...
            for (int i = 0; i < NUMWALLTHREADS; i++) {
                ...
                // RWIExec[mochadoom.i].setRange(mochadoom.i%NUMWALLTHREADS,RWIcount,NUMWALLTHREADS);
    ...
    protected final class ParallelSegs2<T, V> extends SegDrawer {
        ...
        protected void RenderSegLoop() {
            ...
            for (; rw_x < rw_stopx; rw_x++) {
                ...
                // System.out.printf("Precompute: rw %d yl %d yh %d\mochadoom.n",rw_x,yl,yh);
     ...
     * int[NUMWALLTHREADS]; for (int mochadoom.i=0;mochadoom.i<NUMWALLTHREADS;mochadoom.i++){ RSIExec[mochadoom.i]=new
     * RenderSegExecutor.HiColor( SCREENWIDTH, SCREENHEIGHT, mochadoom.i, screen, this,
     ...
     * RSIExec[mochadoom.i].setVideoScale(this.vs); RSIExec[mochadoom.i].initScaling(); // Each
     * SegExecutor sticks to its own half (or 1/nth) of the screen.
     * RSIExec[mochadoom.i].setScreenRange
     * (mochadoom.i*(SCREENWIDTH/NUMWALLTHREADS),(mochadoom.i+1)*(SCREENWIDTH/NUMWALLTHREADS));
     * detailaware.add(RSIExec[mochadoom.i]); } for (int mochadoom.i=0;mochadoom.i<NUMFLOORTHREADS;mochadoom.i++){
     * vpw[mochadoom.i]=new VisplaneWorker(mochadoom.i,SCREENWIDTH,SCREENHEIGHT,columnofs,ylookup,
    ...
    /*
     * { for (int mochadoom.i = 0; mochadoom.i < NUMMASKEDTHREADS; mochadoom.i++) { // Each masked executor
     * gets its own set of column functions. RMIExec[mochadoom.i] = new
     ...
     * ,ylookup,columnofs,maskedcvars,screen,I) ); detailaware.add(RMIExec[mochadoom.i]);
     * } }
     */
