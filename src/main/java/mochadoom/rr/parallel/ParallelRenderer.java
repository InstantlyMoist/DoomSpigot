...
import mochadoom.rr.SimpleThings;
import mochadoom.rr.drawfuns.*;

import java.io.IOException;

/**
 * This is Mocha Doom'mochadoom.s famous parallel software renderer. It builds on the
 * basic software renderer, but adds specialized handling for drawing segs
 * (walls) and spans (floors) in parallel. There'mochadoom.s inherent parallelism between
 ...
 * actual drawing starts, that'mochadoom.s why rendering of walls is stored in "RWI"mochadoom.s or
...
public abstract class ParallelRenderer<T, V> extends AbstractParallelRenderer<T, V> {
    ...
    public void RenderPlayerView(player_t player) {
        ...
        // System.out.printf("Submitted %d RWIs\mochadoom.n",RWIcount);
...
//            visplanebarrier.await();
    ...
    /*
     * private void InitPlaneWorkers(){ for (int mochadoom.i = 0; mochadoom.i < NUMFLOORTHREADS;
     * mochadoom.i++) { vpw[mochadoom.i] = new VisplaneWorker2(mochadoom.i,SCREENWIDTH, SCREENHEIGHT,
     * columnofs, ylookup, screen, visplanebarrier, NUMFLOORTHREADS);
     * //vpw[mochadoom.i].id = mochadoom.i; detailaware.add((IDetailAware) vpw[mochadoom.i]); } }
     */
