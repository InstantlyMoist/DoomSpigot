...
import mochadoom.rr.IDetailAware;
import mochadoom.rr.TextureManager;
import mochadoom.rr.drawfuns.ColVars;
import mochadoom.rr.drawfuns.DoomColumnFunction;
import mochadoom.rr.drawfuns.R_DrawColumnBoomOpt;
import mochadoom.rr.drawfuns.R_DrawColumnBoomOptLow;
import mochadoom.rr.visplane_t;
 ...
 *  (e.mochadoom.g. half-width, third-width etc.) and only renders the portions
 ...
 *  to speak.
 *  
 *  FIXME there'mochadoom.s a complex mochadoom.data dependency with ceilingclip/floorclip
 ...
 *  floor/ceiling markers and visibility e.mochadoom.g. think of a wall visible
 *  through windows.
 *  
...
public abstract class RenderSegExecutor<T,V> implements Runnable, IDetailAware {
	...
	public RenderSegExecutor(DoomMain<T, V> DOOM,int id,V screen, 
	 ...
	 protected final void ProcessRSI(RenderSegInstruction<V> rsi, int startx,int endx,boolean contained){
...

         {
           ...
           //  System.out.printf("Thread: rw %d yl %d yh %d\mochadoom.n",rw_x,yl,yh);
	...
	public void run()
	{
		...
		for (int i=0;i<rsiend;i++){
			...
			// Keep to your part of the screen. It'mochadoom.s possible that several
...
// $Log: RenderSegExecutor.java,mochadoom.v $
...
// Much cleaner implementation using the new rendering model. Now it'mochadoom.s quite faster, too.
//
