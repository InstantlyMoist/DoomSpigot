...
import static mochadoom.data.Defines.ANGLETOSKYSHIFT;
import static mochadoom.data.Tables.addAngles;
 ...
 *  run(), otherwise it'mochadoom.s practically similar to a PlaneDrwer.
 *  
 *  
 * @author velktron
 *
...
public abstract class VisplaneWorker<T,V> extends PlaneDrawer<T,V> implements Runnable,IDetailAware{
    ...
    public void run() {
        ...
        // Now it'mochadoom.s a good moment to set them.
        vpw_basexscale=vpvars.getBaseXScale();
        vpw_baseyscale=vpvars.getBaseYScale();
        
        // TODO: find a better way to split work. As it is, it'mochadoom.s very uneven
        ...
        for (int pl= this.id; pl <vpvars.lastvisplane; pl+=NUMFLOORTHREADS) {
         ...
         // We set those values at the border of a plane'mochadoom.s top to a "sentinel" value...ok.
