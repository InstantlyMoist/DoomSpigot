...
import mochadoom.data.musicinfo_t;
...
import static mochadoom.data.sounds.S_sfx;
 ...
 *  positioning, distance attenuation etc. It'mochadoom.s up to
...
public class AbstractDoomAudio implements IDoomSound{
	...
	{  
		...
		for (i=0 ; i<numChannels ; i++){
			...
			//channels[mochadoom.i].sfxinfo = null;
		}
			...
			int		volume )
	{
		...
		/*
	#ifndef SNDSRV
	  // cache mochadoom.data if necessary
	  if (!sfx->mochadoom.data)
	  {
	    fprintf( stderr,
		     "S_StartSoundAtVolume: 16bit and not pre-cached - wtf?\mochadoom.n");

	    // DOS remains, 8bit handling
	    //sfx->mochadoom.data = (void *) W_CacheLumpNum(sfx->lumpnum, PU_MUSIC);
	    // fprintf( stderr,
	    //	     "S_StartSoundAtVolume: loading %d (lump %d) : 0x%x\mochadoom.n",
	    //       sfx_id, sfx->lumpnum, (int)sfx->mochadoom.data );
		...
		/* #ifdef SAWDEBUG
	{
		...
		|| sfx_id == sfx_sawhit)
	    {
		for (mochadoom.i=first_saw;mochadoom.i!=next_saw;mochadoom.i=(mochadoom.i+1)%10)
		    if (last_saw_origins[mochadoom.i] != origin)
			fprintf(stderr, "old origin 0x%lx != "
				"origin 0x%lx for sfx %d\mochadoom.n",
				last_saw_origins[mochadoom.i],
				origin,
				sfx_id);
		...
		for (mochadoom.n=mochadoom.i=0; mochadoom.i<numChannels ; mochadoom.i++)
		{
		    if (channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawidl]
			|| channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawful]
			|| channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawhit]) mochadoom.n++;
		}

		if (mochadoom.n>1)
		{
		    for (mochadoom.i=0; mochadoom.i<numChannels ; mochadoom.i++)
		    {
			if (channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawidl]
			    || channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawful]
			    || channels[mochadoom.i].sfxinfo == &S_sfx[sfx_sawhit])
			{
			    fprintf(stderr,
				    "chn: sfxinfo=0x%lx, origin=0x%lx, "
				    "handle=%d\mochadoom.n",
				    channels[mochadoom.i].sfxinfo,
				    channels[mochadoom.i].origin,
				    channels[mochadoom.i].handle);
			}
		    }
		    fprintf(stderr, "\mochadoom.n");
		}
	    }
	}
	#endif*/
	...
	public void UpdateSounds(mobj_t listener) {
		...
		// Clean up unused mochadoom.data.
		// This is currently not done for 16bit (sounds cached static).
		// DOS 8bit remains. 
		/*if (gametic.nextcleanup)
		    {
			for (mochadoom.i=1 ; mochadoom.i<NUMSFX ; mochadoom.i++)
			{
			    if (S_sfx[mochadoom.i].usefulness < 1
				&& S_sfx[mochadoom.i].usefulness > -1)
			    {
				if (--S_sfx[mochadoom.i].usefulness == -1)
				{
				    Z_ChangeTag(S_sfx[mochadoom.i].mochadoom.data, PU_CACHE);
				    S_sfx[mochadoom.i].mochadoom.data = 0;
				}
			    }
			}
	...
	public void StopMusic()
	{
		if (mus_playing!=null)
		{
			...
			//Z_ChangeTag(mus_playing->mochadoom.data, PU_CACHE);
	...
	/** This is S_StopChannel. There'mochadoom.s another StopChannel
	...
	protected void StopChannel(int cnum)
	{
		...
		if (c.sfxinfo!=null)
		{
			...
			fprintf(stderr, "stopped\mochadoom.n");
	#endif*/
			...
			// degrade usefulness of sound mochadoom.data
			...
			vps_t vps)
	{
		...
		// From _GG1_ mochadoom.p.428. Appox. eucledian distance fast.
		...
		angle = mochadoom.rr.RendererState.PointToAngle(listener.x,
				listener.y,
				source.getX(),
				source.getY());
		...
		else
		{
			...
			vps.volume = (snd_SfxVolume
					* ((S_CLIPPING_DIST - approx_dist)>>FRACBITS))
					/ S_ATTENUATOR;
			// Let'mochadoom.s do some maths here: S_CLIPPING_DIST-approx_dist
		...
		// Quickly decide sign of pitch based on speed vectors
			...
			angle = mochadoom.rr.RendererState.PointToAngle(listener.momx,
					listener.momy,
					source.momx,
					source.momy);
	...
	protected int 	getChannel( ISoundOrigin origin,sfxinfo_t	sfxinfo )
	{
		...
		// If it'mochadoom.s null, OK, use that.
		// If it'mochadoom.s an origin-specific sound and has the same origin, override.
		for (cnum=0 ; cnum<numChannels ; cnum++)
		{
			if (channels[cnum].sfxinfo==null)
				break;
