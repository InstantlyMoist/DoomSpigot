...
public abstract class AbstractSoundDriver implements ISoundDriver {
    ...
    /**
     * Pitch to stepping lookup, used in ClassicSoundDriver It'mochadoom.s actually rigged
    ...
    protected byte[] getsfx(String sfxname, int[] len, int index) {
        ...
        if (dmx.speed==SAMPLERATE/2){
            ...
            //DSP.filter(dmx.mochadoom.data,SAMPLERATE, SAMPLERATE/4);
        ...
        // MAES: A-ha! So that'mochadoom.s how they do it.
        // SOund effects are padded to the highest multiple integer of
        // the mixing buffer'mochadoom.s size (with silence)
     ...
     * the internal channels. As the SFX info struct contains e.mochadoom.g. a pointer to
     * the raw mochadoom.data it is ignored. As our sound handling does not handle
     * priority, it is ignored. Pitching (that is, increased speed of playback)
     * is set, but whether it'mochadoom.s used or not depends on the final implementation
     * (e.mochadoom.g. classic mixer uses it, but AudioLine-based implementations are not
     * guaranteed.
     */
    ...
    //
    // Retrieve the raw mochadoom.data lump index
    ...
    protected final void initSound8() {
        ...
        for (i = 1; i < NUMSFX; i++) {
            ...
            if (sounds.S_sfx[i].link == null) {
                // Load mochadoom.data from WAD file.
    ...
    /**
     * This is only the common part of InitSound that caches sound mochadoom.data in
