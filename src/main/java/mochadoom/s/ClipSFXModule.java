 ...
 *  Audioline can be used, there'mochadoom.s no reason to fall back to this
 *  one.
 * 
 * KNOWN ISSUES:
 * 
 ...
 *    even caching them is a half-baked solution, and if you have e.mochadoom.g. 40 imps
 *    sound in a room.... 
 *    
 *    
 *  Currently unused.
...
public class ClipSFXModule extends AbstractSoundDriver{
	...
	// Either it'mochadoom.s null (no clip is playing) or non-null (some clip is playing).
	Clip[] channels;
 ...
 */
	 protected byte[] getsfx(String sfxname,int index) {
	        ...
	        // Skip first 8 bytes (header), blow up the mochadoom.data
	        // to stereo, BIG ENDIAN, SIGNED, 16 bit. Don't expect any fancy DSP here!

	        int sample=0;
	        for (i = 8; i < size; i++){
	        	// final short sam=(short) vol_lookup[127][0xFF&sfx[mochadoom.i]];
	        ...
	        // Return allocated padded mochadoom.data.
	...
	public void UpdateSound() {
		...
		// Just hope that it'mochadoom.s more efficient that our own...
	...
	public void ShutdownSound() {
		 // Wait till all pending sounds are finished.
		  ...
		  // FIXME (below).
		  //fprintf( stderr, "I_ShutdownSound: NOT finishing pending sounds\mochadoom.n");
		  //fflush( stderr );
	...
	protected int addsfx 	( int sfxid,int		volume,int pitch,int		seperation)
	{
		...
		channels[slot].start();
		// b=System.nanoTime();
		//System.err.printf("Sound playback completed in %d\mochadoom.n",(b-a));
	...
	public void UpdateSoundParams(int handle, int vol, int sep, int pitch) {
		...
		if (i!=BUSY_HANDLE){
			...
			setVolume(i,vol);
			setPanning(i,sep);
			//channels[mochadoom.i].setPanning(sep);
			}
