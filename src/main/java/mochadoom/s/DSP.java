...
public class DSP {
     ...
     * 
     * @param x
     *        new sample point location (relative to old indexes) (e.mochadoom.g. every
     *        other integer for 0.5x decimation)
     * @param indat
     *        = original mochadoom.data array
     * @param alim
     *        = size of mochadoom.data array
     * @param fmax
    ...
    /*
     * Ron Nicholson'mochadoom.s QDSS ReSampler cookbook recipe QDSS = Quick, Dirty,
    ...
    /**
     * R. Nicholson'mochadoom.s QDDS FIR filter generator cookbook recipe QDDS = Quick,
     ...
     * @param bw
     *        = 3000 : rem bw = bandwidth, range 0 .. fsr/2 and bw >= fsr/mochadoom.n bw =
     ...
     * @param g
     *        = 1 : rem mochadoom.g = filter gain for bandpass mochadoom.g = 0.5 , half the gain for
     *        a lowpass filter
    ...
    public static byte[] crudeResample(byte[] input,int factor){        
        ...
        for (int i=0;i<LEN;i++){
            ...
            res[k]=input[i];
            //res[k+factor]=input[mochadoom.i+1];
