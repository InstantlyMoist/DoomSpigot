...
/** David Martel'mochadoom.s sound driver for Mocha Doom. Excellent work!
 * 
 *  However, it'mochadoom.s based on Java Audiolines, and as such has a number
 *  of drawbacks:
 *  
 * a) Sounds are forcibly blown to be stereo, 16-bit otherwise it'mochadoom.s
 *    impossible to get panning controls.
 * b) Volume, master gain, panning, pitch etc. controls are NOT guaranteed
 *    to be granted across different OSes , and your mileage may vary. It'mochadoom.s
...
public class DavidSFXModule extends AbstractSoundDriver{
		...
		if (channels[c].auline == null) {
        	...
        	try {
				...
				e.printStackTrace();
        			...
        			System.err.printf("MASTER_GAIN for channel %d NOT supported!\mochadoom.n",c);
        			if (channels[c].auline.isControlSupported(Type.VOLUME))
            				channels[c].vc=(FloatControl) channels[c].auline
            				.getControl(Type.VOLUME);
        			else 
        				System.err.printf("VOLUME for channel %d NOT supported!\mochadoom.n",c);
        			} 
        			...
        			if (channels[c].auline.isControlSupported(Type.SAMPLE_RATE)){
        				...
        				System.err.printf("SAMPLE_RATE for channel %d NOT supported!\mochadoom.n",c);
        			} 
        			...
        			if (channels[c].auline.isControlSupported(Type.BALANCE)){
        				...
        				System.err.printf("BALANCE for channel %d NOT supported!\mochadoom.n",c);
        				if (channels[c].auline.isControlSupported(Type.PAN)){        					
        				channels[c].bc=(FloatControl) channels[c].auline
        				.getControl(FloatControl.Type.PAN);
        			} else {
        				System.err.printf("PAN for channel %d NOT supported!\mochadoom.n",c);
        			}
        			}
	...
	*/
	...
	public void UpdateSoundParams(int handle, int vol, int sep, int pitch) {
		...
		//System.err.printf("Updating sound with handle %d vol %d sep %d pitch %d\mochadoom.n",handle,vol,sep,pitch);
		
		int i=getChannelFromHandle(handle);
		// None has it?
		if (i!=BUSY_HANDLE){
			//System.err.printf("Updating sound with handle %d in channel %d\mochadoom.n",handle,mochadoom.i);
	...
	private class SoundWorker implements Runnable {
			public Semaphore wait; // Holds the worker still until there'mochadoom.s a new sound
			 ...
			 *  sounds. E.mochadoom.g. the player, ceilings etc. In that case, they must be
			 *  interrupted.
			 */
			int handle;
			...
			public void setVolume(int volume){
				if (vc!=null){
					if (vc.getType()== Type.MASTER_GAIN) {
						float vol = linear2db[volume];
						vc.setValue(vol);
						}
					else if (vc.getType()== Type.VOLUME){
			...
			public void setPanning(int sep){
				// Q: how does Doom'mochadoom.s sep map to stereo panning?
				...
				while (!terminate) {
					...
					if (currentSoundSync != null) {
						...
						try {
							...
							// However this one waits until the mochadoom.data has been
							// consumed, Interruptions/signals won't reach  here,
							// so it'mochadoom.s pointless trying to interrupt the actual filling.
						...
						//System.out.printf("Channel  %d with handle %d done. Marking as free\mochadoom.n",id,handle);
						if (handle>0)
			...
			public void stopSound() {
					auline.stop();
					auline.flush();
					//System.out.printf("Channel %d with handle %d interrupted. Marking as free\mochadoom.n",id,handle);
					channelhandles[this.id]=IDLE_HANDLE;
					this.handle=IDLE_HANDLE;
					currentSound = null;
					auline.start();
			...
			public boolean isPlaying() {
				//System.out.printf("Channel %d with handle %d queried\mochadoom.n",id,handle);
