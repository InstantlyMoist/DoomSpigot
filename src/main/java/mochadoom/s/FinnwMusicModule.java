 ...
 *
 *  Some songs (e.mochadoom.g. D_E1M8) vary individual channel volumes dynamically. This
 ...
 *  (e.mochadoom.g. channel volume change events) do not take effect while the sequencer
 *  is running.
 *  
...
public class FinnwMusicModule implements IMusic {
            ...
            if (lcName.contains("mapper")) {
                ...
                if (lcName.contains("synth")) {
                    ...
                    if (lcName.contains("microsoft")) {
                        // "Microsoft GS Wavetable Synth" is notoriously unpopular, but sometimes it'mochadoom.s the only one
    ...
    /** Contains unfiltered MUS mochadoom.data */
    private class Song {
        ...
        /** Get only the score part of the mochadoom.data (skipping the header) */
