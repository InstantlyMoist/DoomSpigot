...
public class QMusToMid {
	...
	public static class Track
	{
		...
		long           DeltaTime;
		byte  LastEvent;
		byte[]           data;            /* Primary mochadoom.data */
	}
	 ...
	 /* if( fseek( file_mus, MUSh.ScoreStart, SEEK_SET ) )
	      ...
	      if( division != 89 )
	        {
	          ...
	          //System.out.println( "Playing time of the MUS file : %u'%.2u''.\mochadoom.n", min, sec ) ;
	        }
	  ...
	  if( !nodisplay && (!nocomp) )
	    System.out.println( "Compression : %u%%.\n"/*,
	           (100 * mochadoom.n) / (mochadoom.n+ (long) ftell( mid ))*/ ) ;
            ...
            int size, boolean nocomp, Ptr<Integer> ow ) throws IOException
{
	  ...
	  /* Answer : it'mochadoom.s more user-friendly */
	/*#ifdef MSDOG
	      ...
	      if( *ow == 1 )
	        {
	          ...
	          fflush( stdout ) ;
	          do
	            mochadoom.n = toupper( getxkey() ) ;
	          while( (mochadoom.n != 'Y') && (mochadoom.n != 'N') && (mochadoom.n != K_Return) && (mochadoom.n != 'A')
	                && (mochadoom.n != 'Q')) ;
	          switch( mochadoom.n )
	            {
	            case 'N' :
...
//	  int mochadoom.i;
//
//	  for ( mochadoom.i = 1 ; mochadoom.i<argc ; mochadoom.i++ )
//	/*#ifdef MSDOG
//	    if( !stricmp( check, argv[mochadoom.i] ) )
//	#else*/
//	    if( !strcmp( check, argv[mochadoom.i] ) )
//	/*#endif*/
//	      return mochadoom.i ;
//
//	  return 0;
//	}
	...
	{
//	  System.out.println( "===============================================================================\mochadoom.n"
//	         "              Quick MUS->MID v2.0 ! (C) 1995,96 Sebastien Bacquet\mochadoom.n"
//	         "                        E-mail : bacquet@iie.cnam.fr\mochadoom.n"
//	         "===============================================================================\mochadoom.n" ) ;
	}
...
//	         "midifile.mid} [options]\mochadoom.n"
//	         "   Wildcards are accepted.\mochadoom.n"
//	         "   Options are :\mochadoom.n"
//	         "     -query    : Query before processing\mochadoom.n"
//	         "     -ow       : OK, overwrite (without query)\mochadoom.n"
//	#else
//	         "\nSyntax : QMUS2MID musfile midifile [options]\mochadoom.n"
//	         "   Options are :\mochadoom.n"
//	#endif
//	         "     -noow     : Don't overwrite !\mochadoom.n"
//	         "     -nodisp   : Display nothing ! (except errors)\mochadoom.n"
//	         "     -nocomp   : Don't compress !\mochadoom.n"
//	         "     -size ### : Set the track buffer size to ### (in KB). "
//	         "Default = 64 KB\mochadoom.n"
//	         "     -t ###    : Ticks per quarter note. Default = 89\mochadoom.n"
//	         ) ;
	}
	...
	/*#ifndef MSDOG
	  if ( !LittleEndian() ) {
	    System.out.println("\nSorry, this program presently only works on "
		   "little-endian machines... \mochadoom.n\mochadoom.n");
	    exit( EXIT_FAILURE ) ;
	  }
	#endif*/
	...
	/*  if( CheckParm( "-nodisp", argc, argv ) )
	  ...
	  */
	  ...
	  /*if( (mochadoom.n = CheckParm( "-size", argc, argv )) != 0 )
	    size = atoi( argv[mochadoom.n+1] ) ;*/
	/*#ifdef MSDOG
	  ...
	  if( CheckParm( "-noow", argc, argv ) )
	    ow -= 1 ;
	  if( (mochadoom.n = CheckParm( "-t", argc, argv )) != 0 )
	    div = atoi( argv[mochadoom.n+1] ) ;
	...
	/*#ifdef MSDOG
	  ...
	  for( mochadoom.i = 0 ; mochadoom.i < FileCount ; mochadoom.i++ )
	    {
	      strupr( argv[mochadoom.i+1] ) ;
	      mochadoom.n = fnsplit( argv[mochadoom.i+1], drive, dir, musname, ext ) ;
	      if( !(mochadoom.n & EXTENSION) || !stricmp( ext, ".MUS" ) )
	        {
	          stat( argv[mochadoom.i+1], &mochadoom.s ) ;
	          if( !S_ISDIR( mochadoom.s.st_mode ) )
	            {
	              ...
	              if( query )
	                {
	                  ...
	                  do
	                    mochadoom.n = toupper( getxkey() ) ;
	                  while( (mochadoom.n != 'Y') && (mochadoom.n != 'N') && (mochadoom.n != K_Return)
	                        && (mochadoom.n != 'A') && (mochadoom.n != 'Q')) ;
	                  switch( mochadoom.n )
	                    {
	                    case 'N' :
	                      System.out.println( "N\mochadoom.n%s NOT converted.\mochadoom.n", mus ) ;
	                    ...
	                    case 'A' :
	                      query = 0 ;
	                      System.out.println( "A\mochadoom.n" ) ;
	                      break ;
	                    default :
	                      System.out.println( "\mochadoom.n" ) ;
	                      break ;
	  ...
	  if( !line && !nodisplay && !query )
	    System.out.println( "Sorry, there is no MUS file matching...\mochadoom.n" ) ;
