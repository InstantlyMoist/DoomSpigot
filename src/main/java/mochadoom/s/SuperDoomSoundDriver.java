...
import mochadoom.pooling.AudioChunkPool;
 ...
 * 
 * CONS:
 * a) All those of ClassicSoundDriver plus regarding mochadoom.timing accuracy.
 * 
 * @author Maes
 */
...
public class SuperDoomSoundDriver extends AbstractSoundDriver {
    ...
    /** These are still defined here to decouple them from the mixer'mochadoom.s
     ...
     * given number of samples from the raw sound mochadoom.data, modifies it according to
    ...
    public void SetChannels(int numChannels) {
        // Init internal lookups (raw mochadoom.data, mixing buffer, channels).
    ...
    public boolean InitSound() {
        ...
        // We only need a single mochadoom.data line.
        ...
        if (AudioSystem.isLineSupported(info))
            try {
                ...
                e.printStackTrace();
                System.err.print("Could not play signed 16 mochadoom.data\n");
                return false;
            }
        ...
        // Initialize external mochadoom.data (all sounds) at start, keep static.
        System.err.print("I_InitSound: ");

        super.initSound8();

        System.err.print("pre-cached all sound mochadoom.data\n");
    ...
    protected int addsfx(int sfxid, int volume, int step, int seperation) {
        ...
        // Set pointer to raw mochadoom.data.
        channels[slot]=true;
        m.channel=slot;
        ...
        // Set pointer to end of raw mochadoom.data.
        m.end = lengths[sfxid];

        // Reset current handle number, limited to 0..100.
        if (handlenums == 0) // was !handlenums, so it'mochadoom.s actually 1...100?
            handlenums = 100;
        ...
        // e.mochadoom.g. for avoiding duplicates of chainsaw.
    ...
    public void ShutdownSound() {
        ...
        // Unlock sound thread if it'mochadoom.s waiting.
        produce.release();
    ...
    protected class PlaybackServer
        ...
        public void run() {
            ...
            while (!terminate) {
                ...
                // while (mochadoom.timing[mixstate]<=mytime){
                ...
                try {
                    ...
                    produce.acquire();
                    //System.err.print("...got it\mochadoom.n");
                } catch (InterruptedException e) {
                    // Well, ouch.
                    e.printStackTrace();
                }
                ...
                // System.err.printf("Audio queue has %d chunks\mochadoom.n",audiochunks.size());
    ...
    protected class MixMessage {
    	 ...
    	 * Therefore pointer, remainder and mochadoom.data should remain untouched.
    ...
    implements Runnable {
		...
		public void run()  {
	        ...
	        // Mix current sound mochadoom.data.
	        ...
	        while (!terminate) {
	        	...
	        	// rather than choppy interruptions.
	        		...
	        		try {
						...
						//System.err.print("...broke free\mochadoom.n");
					} catch (InterruptedException e) {
	        ...
	        while (leftout < leftend) {
	            ...
	            for (chan = 0; chan < numChannels; chan++) {
	                ...
	                // MAES: this means that we must point to raw mochadoom.data here.
	                if (channels[chan] != null) {
	                    int channel_pointer = p_channels[chan];

	                    // Get the raw mochadoom.data from the channel.
	                    ...
	                    // to the current mochadoom.data. Adjust volume accordingly.
	       ...
	       // for (chan = 0; chan < numChannels; chan++) {
	       // 	if (channels[chan]!=null){
	       // 		System.err.printf("Channel %d pointer %d\mochadoom.n",chan,this.p_channels[chan]);
	       // 	}
	       // }
		...
		private AudioChunk gunk;
    		...
    		private final void submitSound(){
    			// It'mochadoom.s possible to stay entirely silent and give the audio
    	        // queue a chance to get drained. without sending any mochadoom.data.
    			// Saves BW and CPU cycles.
    	        if (mixed) {
    	            silence=0;


    	            // System.err.printf("Submitted sound chunk %d to buffer %d \mochadoom.n",chunk,mixstate);
	        ...
	        private final void stopChannel(int channel){
	        	//System.err.printf("Stopping channel %d\mochadoom.n",channel);
	        	this.channels[channel]=null;
    			this.p_channels[channel]=0;
	        	}
	        
	        private final void updateChannel(MixMessage m){
	        	//System.err.printf("Updating channel %d\mochadoom.n",mochadoom.m.channel);
	        ...
	        private final void insertChannel(MixMessage m){
	        	int ch=m.channel;
	        	//System.err.printf("Inserting channel %d\mochadoom.n",ch);
    ...
    // so that it doesn't outrun the audioline'mochadoom.s buffer and game updates.
