...
// -----------------------------------------------------------------------------
//
// $Id: StatusBar.java,mochadoom.v 1.47 2011/11/01 23:46:37 velktron Exp $
//
...
import mochadoom.data.sounds.musicenum_t;
import mochadoom.defines.*;
import mochadoom.doom.*;
import mochadoom.doom.SourceCode.CauseOfDesyncProbability;
import mochadoom.doom.SourceCode.ST_Stuff;
import mochadoom.g.Signals;
import mochadoom.m.Settings;
import mochadoom.m.cheatseq_t;
...
import static mochadoom.data.Limits.MAXPLAYERS;
import static mochadoom.data.Tables.*;
import static mochadoom.doom.SourceCode.ST_Stuff.ST_Responder;
import static mochadoom.doom.englsh.*;
import static mochadoom.doom.items.*;
import static mochadoom.doom.player_t.*;
...
public class StatusBar extends AbstractStatusBar {
    ...
    // used for mochadoom.timing (unsigned int .. maybe long !)
    ...
    // whether in mochadoom.automap or first-person
    ...
    // Filter mochadoom.automap on/off.
    ...
    public void NotifyAMExit() {
        // fprintf(stderr, "AM exited\mochadoom.n");
    ...
    public boolean Responder(event_t ev) {
        if (ev.isType(evtype_t.ev_keydown)) {
            ...
            if (ev.ifKeyAsciiChar(cheat_clev::CheckCheat)) {
                ...
                // MAES: If it'mochadoom.s mochadoom.doom.wad but not ultimate
     ...
     * mochadoom.timing. the precedence of expressions is: dead > evil grin > turned head
     * > straight ahead
     */
                    ...
                    : plyr.health[0] - st_oldhealth) > ST_MUCHPAIN)
                {
                    ...
                    badguyangle =
                        DOOM.sceneRenderer.PointToAngle2(plyr.mo.x, plyr.mo.y, plyr.attacker.x,
                            plyr.attacker.y);
                    boolean obtuse; // that'mochadoom.s another "mochadoom.i"
    ...
    protected int largeammo = 1994; // means "mochadoom.n/a"
    ...
    public void updateWidgets() {
        ...
        // if (w_ready.mochadoom.data != plyr.readyweapon)
        // {
    ...
    public void unloadGraphics() {
       ...
       // unload tall percent
         ...
         // unload gray #'mochadoom.s
          ...
          // unload the key cards for (mochadoom.i=0;mochadoom.i<NUMCARDS;mochadoom.i++)
     ...
     * limitations in Java (e.mochadoom.g. passing an array AND an index).
     */
    ...
    class st_multicon_t
        ...
        public void update(boolean refresh) {
            ...
            // Actual value to be considered. Poor man'mochadoom.s generics!
            int thevalue = -1;
            switch (status) {
            case 0:
            ...
            if (this.on[onindex] && ((this.oldinum != thevalue) || refresh)
                ...
                if (this.oldinum != -1) { 
                    ...
                    //System.out.printf("Restoring at x y %d %d mochadoom.w h %d %d\mochadoom.n",x, y - ST_Y,mochadoom.w,h);
                    DOOM.graphicSystem.CopyRect(SB, rect, FG, DOOM.graphicSystem.point(x, y));
                    //V.CopyRect(x, y - ST_Y, SCREEN_SB, mochadoom.w, h, x, y, SCREEN_FG);
                    //V.FillRect(x, y - ST_Y, mochadoom.w, h, FG);
                }
                
                //System.out.printf("Drawing at x y %d %d mochadoom.w h %d %d\mochadoom.n",this.x,this.y,mochadoom.p[thevalue].width,mochadoom.p[thevalue].height);
    ...
    class st_number_t
        ...
        public void drawNum(boolean refresh) {
            ...
            if (this.y - ST_Y < 0) {
                DOOM.doomSystem.Error("drawNum: mochadoom.n.y - ST_Y < 0");
            }

            // Restore BG from buffer
            //V.FillRect(x+(numdigits-3) * mochadoom.w, y, mochadoom.w*3 , h, FG);
            ...
            //V.CopyRect(x+(numdigits-3)*mochadoom.w, y- ST_Y, SCREEN_SB, mochadoom.w * 3, h, x+(numdigits-3)*mochadoom.w, y, SCREEN_FG);
            ...
            if (num == 0)
                //V.DrawPatch(x - mochadoom.w, mochadoom.n.y, FG, mochadoom.n.mochadoom.p[0]);
            ...
            while (((num != 0) && (numdigits-- != 0))) {
                x -= w;
                //V.DrawPatch(x, mochadoom.n.y, FG, mochadoom.n.mochadoom.p[num % 10]);
            ...
            if (neg)
                ...
                //V.DrawPatch(x - sttminus.width*vs.getScalingX(), mochadoom.n.y, FG, sttminus);
        }
...
//$Log: StatusBar.java,mochadoom.v $
...
//Now using mochadoom.g.Keys for key input stuff.
//
...
//Added V_SCALEOFFSET flag to doomguy'mochadoom.s mug.
//
