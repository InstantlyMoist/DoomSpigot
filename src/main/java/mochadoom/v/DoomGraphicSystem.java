 ...
 *  must implement. 
 *  
 *  23/10/2011: Made into a generic type, which affects the underlying raw screen mochadoom.data
 ...
 *  be aware of the underlying implementation. E.mochadoom.g. the various screen arrays will not be
 *  necessarily byte[].
 * 
 * @author Maes
 */
...
public interface DoomGraphicSystem<T, V> {
    ...
    final int V_NOSCALEOFFSET     = 0x02000000;   // dont'mochadoom.s cale patch offset
     ...
     * must always be able to "tap" into what'mochadoom.s the current, "correct" screen after all manipulation and
