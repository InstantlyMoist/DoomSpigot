...
public interface Lights extends Colors {
     ...
     * for example it could be included in mochadoom.savegame or demos.
     * 
     * If we preshift inverse colormap, MochaDoom not in TrueColor bppMode or any Vanilla DOOM would crash
     * when trying to load mochadoom.savegame made when under invulnerabilty in TrueColor bppMode.
    ...
    default int[][] BuildLights24(int[] palette, byte[][] colormap) {
        ...
        for (int i = 1; i < COLORMAP_LIGHTS_24; ++i) {
            // [1 .. 31] the index of the colormap to be target for gradations: max 31 of ceiling of mochadoom.i / 8
            ...
            for (int j = 0; j < PAL_NUM_COLORS; ++j) {
                // translated indexed color from wad-read colormap mochadoom.i at position j
