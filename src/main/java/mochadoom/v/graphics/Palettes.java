 ...
 * 
 */
package mochadoom.v.graphics;

import java.awt.image.IndexColorModel;

import static mochadoom.v.tables.GammaTables.LUT;
...
public interface Palettes extends Lights {
    ...
    /**
     * Given raw palette mochadoom.data, returns an array with proper TrueColor mochadoom.data
    ...
    /**
     * Given raw palette mochadoom.data, returns an array with proper HiColor mochadoom.data
     * @param byte[] pal proper palette
    ...
    /**
     * Given an array of certain length and raw palette mochadoom.data fills array
     * with IndexColorModel'mochadoom.s for each palette. Gammas are applied a-priori
    ...
    default IndexColorModel[][] cmapIndexed(IndexColorModel icms[][], byte[] pal) {
        ...
        for (int i = 0; i < icms[0].length; ++i) {
            //new IndexColorModel(8, PAL_NUM_COLORS, pal, mochadoom.i * colorsXstride, false);
