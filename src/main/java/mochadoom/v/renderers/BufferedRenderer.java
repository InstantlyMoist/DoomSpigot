...
class BufferedRenderer extends SoftwareIndexedVideoRenderer {
    ...
    /**
     * This actually creates a raster with a fixed underlying array, but NOT the images themselves. So it'mochadoom.s possible to
    ...
    BufferedRenderer(RendererFactory.WithWadLoader<byte[], byte[]> rf) {
        ...
        for (DoomScreen s: DoomScreen.values()) {
            ...
            // Only create non-visible mochadoom.data, pegged to the raster. Create visible images only on-demand.
...
//$Log: BufferedRenderer.java,mochadoom.v $
