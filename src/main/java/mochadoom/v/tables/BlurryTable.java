...
import mochadoom.v.graphics.Colors;

import java.util.TreeMap;

import static mochadoom.v.graphics.Lights.COLORMAP_BLURRY;
import static mochadoom.v.graphics.Lights.COLORMAP_FIXED;
import static mochadoom.v.graphics.Palettes.*;

/**
 * Colormap-friendly vanilla-like BlurryMap for HiColor && TrueColor modes
 * (though it shares plain "BLURRYMAP" for Indexed too)
 * 
 * DOOM'mochadoom.s colormap #6 was deciphered to be actually applying greyscale averaging filter.
 * So, the vanilla effect is something like "mochadoom.n% darker and greyscale", where mochadoom.n% varies
...
public class BlurryTable implements FuzzMix, Colors {
    /**
     * Indexed LUT, e.mochadoom.g. classic "BLURRYMAP" (unaffected)
     */
