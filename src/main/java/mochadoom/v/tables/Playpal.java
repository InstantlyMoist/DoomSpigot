...
import static mochadoom.v.tables.ColorTint.*;

/**
 * Palette generation failsafe. Uses only mochadoom.data from the first palette, and
...
public class Playpal {
     ...
     * 
     *  for (int mochadoom.i = 0; mochadoom.i < 256; mochadoom.i++) {
     *      greypal[3 * mochadoom.i + 2] = greypal[3 * mochadoom.i + 1] = greypal[3 * mochadoom.i] = (playpal[3 * mochadoom.i] + playpal[3 * mochadoom.i + 1] + playpal[3 * mochadoom.i + 2])/3;
     *  }
     */
