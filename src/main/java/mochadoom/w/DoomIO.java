...
//$Id: DoomIO.java,mochadoom.v 1.3 2013/06/03 10:30:20 velktron Exp $

import mochadoom.m.Swap;
...
* it'mochadoom.s possible to change the underlying implementation without (?) changing too
* much of the exposed interface, but it'mochadoom.s not a priority for me right now.
* 
*/
public class DoomIO  {
 ...
 */
   ...
   public static void writeString(DataOutputStream dos,String s,int len) throws IOException {
       ...
       if (s.length() != 0){
           ...
           // Fill in with 0s if something'mochadoom.s left.
           if (dest.length<len){
