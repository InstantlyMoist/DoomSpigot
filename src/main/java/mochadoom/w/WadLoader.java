...
// -----------------------------------------------------------------------------
//
// $Id: WadLoader.java,mochadoom.v 1.64 2014/03/28 00:55:32 velktron Exp $
//
...
import mochadoom.doom.SourceCode;
import mochadoom.doom.SourceCode.W_Wad;
import mochadoom.i.DummySystem;
import mochadoom.i.IDoomSystem;
import mochadoom.Loggers;
import mochadoom.rr.patch_t;
import mochadoom.utils.C2JUtils;
import mochadoom.utils.GenericCopy.ArraySupplier;
...
import static mochadoom.data.Defines.PU_CACHE;
import static mochadoom.doom.SourceCode.W_Wad.W_CacheLumpName;
import static mochadoom.doom.SourceCode.W_Wad.W_CheckNumForName;
import static mochadoom.utils.GenericCopy.malloc;
...
public class WadLoader implements IWadLoader {
	 ...
	 * be a return type.
	 * 
	 * TODO: maybe move this in mochadoom.utils?
	 */
	...
	private void AddFile(String uri,ZipEntry entry,int type) throws Exception {
		...
		// System.out.println(" adding " + filename + "\mochadoom.n");
		...
		if (!C2JUtils.checkForExtension(checkname,"wad")) {
			...
			// implement reading something from RAF into Doom'mochadoom.s structs.
			...
			// MAES: lum_p was an alias for lumpinfo[startlump]. I know it'mochadoom.s a
	 ...
	 *  STATED contents, in case it'mochadoom.s not possible to get an accurate stream size otherwise.
    ...
    /* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#Reload()
	 */
	@Override
	...
	public void Reload() throws Exception {
		...
		for (i = reloadlump; i < reloadlump + lumpcount; i++, lump_p++, fileinfo_p++) {
			if (lumpcache[i] != null) {
				// That'mochadoom.s like "freeing" it, right?
				lumpcache[i] = null;
				preloaded[i] = false;
			}
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#InitMultipleFiles(java.lang.String[])
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#InitFile(java.lang.String)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#NumLumps()
	 */
	@Override
	 ...
	 * could be used). It'mochadoom.s faster than the old method, but
	 * still short from the HashMap'mochadoom.s performance by
	 ...
	 * UNUSED
		...
		while (lump_p-- != 0)
			if (lumpinfo[lump_p].stringhash == hash) {
				// System.out.print(" found "+lumpinfo[lump_p]+"\mochadoom.n" );
				return lump_p;
			}
	...
	/**
	 * Old, shitty method for CheckNumForName. It'mochadoom.s an overly literal
	 * translation of how the C original worked, which was none too good 
	 * even without the overhead of converting a string to
	 * its integer representation. It'mochadoom.s so bad, that it'mochadoom.s two orders
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#GetLumpinfoForName(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#GetNumForName(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#GetNameForNum(int)
	 */
    @Override
	...
	//
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#LumpLength(int)
	 */
	@Override
	...
	public final void ReadLump(int lump, byte[] buf, int offset) {
		...
		try {
			...
			// Well, that'mochadoom.s a no-brainer.
	 ...
	 *  DoomBuffer object in its place, with the raw byte contents. It'mochadoom.s
	 *   
	 * 
	 */
		...
		if ((lumpcache[lump] == null)||(what==null)) {
			...
			if (what != null) {
				try {
					...
					if (implementsInterface(what, mochadoom.w.CacheableDoomObject.class)) {
	 ...
	 *  
	 *  It possible to do this in other ways, but it'mochadoom.s extremely convenient this way.
	 *  
	 ...
	 *  since their size/lump/number relationship is well-defined.
	 *  
	 *  It possible to do this in other (more verbose) ways, but it'mochadoom.s
	 ...
	 *  method in mochadoom.v 1.43 of WadLoader, which is deprecated.
	 *  
	 *  @param lump The lump number to load.
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CacheLumpNameAsRawBytes(java.lang.String, int)
	 */
	...
	public byte[] CacheLumpNameAsRawBytes(String name, int tag) {
	 ...
	 /* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CacheLumpNumAsRawBytes(int, int)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CacheLumpName(java.lang.String, int)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CachePatchName(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CachePatchName(java.lang.String, int)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CachePatchNum(int, int)
	 */
	...
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#CacheLumpName(java.lang.String, int, java.lang.Class)
	 */
	@Override
	...
	void Profile() throws IOException {
		...
		Object ptr;
		char ch;
		FileWriter mochadoom.f;
		int j;
		String name;

		for (mochadoom.i = 0; mochadoom.i < numlumps; mochadoom.i++) {
			ptr = lumpcache[mochadoom.i];
			if ((ptr == null)) {
				ch = ' ';
				continue;
			} else {
			...
			info[mochadoom.i][profilecount] = ch;
		}
		profilecount++;

		mochadoom.f = new FileWriter(new File("waddump.txt"));
		// name[8] = 0;

		for (mochadoom.i = 0; mochadoom.i < numlumps; mochadoom.i++) {
			name = lumpinfo[mochadoom.i].name;

			mochadoom.f.write(name);

			for (j = 0; j < profilecount; j++)
				mochadoom.f.write("    " + info[mochadoom.i][j]);

			mochadoom.f.write("\mochadoom.n");
		}
		mochadoom.f.close();
	} */
	
	/* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#isLumpMarker(int)
	 */
	@Override
	public boolean isLumpMarker(int lump){
	   ...
	   /* (non-Javadoc)
	 * @see mochadoom.w.IWadLoader#GetNameForLump(int)
	 */
	@Override
	 ...
	 * than just using the language'mochadoom.s built-in hash table. Lesson learned, kids:
	...
	protected void InitLumpHash() {
		...
		//for (int mochadoom.i = 0; mochadoom.i < numlumps; mochadoom.i++)
		//	lumpinfo[mochadoom.i].index = -1; // mark slots empty
	 ...
	 * (non-Javadoc)
	 * 
	 * @see mochadoom.w.IWadLoader#CheckNumForName(java.lang.String)
	 */
    @Override
	...
	public int CheckNumForName(String name/* , int namespace */) {
		...
		// System.out.print(" found "+lumpinfo[mochadoom.i]+"\mochadoom.n" );
		return -1;
	}
	...
	public int[] CheckNumsForName(String name) {
		...
		// Dumb search, no chained hashtables I'mochadoom.m afraid :-/
	...
	/** 
	 * Based on Boom'mochadoom.s W_CoalesceMarkedResource
	 * Sort of mashes similar namespaces together so that they form
	 * a continuous space (single start and end, e.mochadoom.g. so that multiple
	 ...
	 * 
	 * It'mochadoom.s convenient for sprites, but can be replaced by alternatives
	 * for flats.
	 * 
...
//	    			start_marker,mochadoom.i);
	...
	{
		// Safeguard against nameless marker lumps e.mochadoom.g. in Galaxia.wad
	...
	public void UnlockLumpNum(CacheableDoomObject lump){
		...
		if (lumpno!=null) {
			...
			//System.out.printf("Lump %d %d freed\mochadoom.n",lump.hashCode(),lumpno);
		}
	}
...
//$Log: WadLoader.java,mochadoom.v $
...
//DoomFile model abandoned. Now streams are used whenever possible, with possible future expandability to use e.mochadoom.g. URL streams or other types of resources other than RandomAccessFiles.
//
...
//Changed default read buffer behavior. Now it'mochadoom.s ALWAYS reset when reading from disk, and not up to the CacheableDoomObject. This does not affect bulk/stream reads.
//
...
//Fixed hashtable bug. Now using Java'mochadoom.s one, faster AND easier to follow.
//
...
//Menu mostly working, implemented Killough'mochadoom.s fast hash-based GetNumForName, although it can probably be finetuned even more.
//
...
//Let'mochadoom.s see if this stuff will finally commit....
//
//
...
//and there'mochadoom.s still mixed C code in there. I suggest you load everything up in
